...
(function () {
    ...
    function renderGlobalButtons() {
        if (document.getElementById("global-schooling-assigner")) return;

        const headings = document.querySelectorAll("h3");
        let heading = null;
        for (const h of headings) {
            if (h.textContent.toLowerCase().includes("personal auswählen")) {
                heading = h;
                break;
            }
        }
        ...
        wrapper.id = "global-schooling-assigner";
        ...
        resetButton.title = "Alle sichtbaren zurücksetzen";
        resetButton.addEventListener("click", async () => {
            const elements = document.querySelectorAll(".personal-select-heading[building_id]");
            for (const el of elements) {
                // Nur sichtbare Gebäude
                if (el.closest(".building_list")?.offsetParent !== null) {
                    const id = el.getAttribute("building_id");
                    await resetPersonnel(id);
                }
            }
        });
        ...
        for (let i = 1; i <= 10; i++) {
            ...
            button.title = `Alle sichtbaren Gebäude: ${i} Personen auswählen`;
            button.addEventListener("click", async () => {
                const elements = document.querySelectorAll(".personal-select-heading[building_id]");
                for (const el of elements) {
                    // Nur sichtbare Gebäude auswählen
                    if (el.closest(".building_list")?.offsetParent !== null) {
                        const id = el.getAttribute("building_id");
                        await selectPersonnel(id, i);
                    }
                }
            });
        ...
        // --- Optionaler Button bei mehr als 10 Personen ---
        if (requiredNumberOfPersonnel > 10) {
            const button = document.createElement("button");
            button.type = "button";
            button.className = "btn btn-default btn-sm schooling-personnel-select-button";
            button.textContent = requiredNumberOfPersonnel.toString();
            button.title = `Alle sichtbaren Gebäude: ${requiredNumberOfPersonnel} Personen auswählen`;
            ...
            button.addEventListener("click", async () => {
                ...
                for (const el of elements) {
                    ...
                    if (el.closest(".building_list")?.offsetParent !== null) {
                        const id = el.getAttribute("building_id");
                        await selectPersonnel(id, requiredNumberOfPersonnel);
                    }
                }
            });
